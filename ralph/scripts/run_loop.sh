...
#   ./ralph/scripts/run_loop.sh T002-something      # 특정 티켓
#   ./ralph/scripts/run_loop.sh --safe-only         # safe: true 만
#   ./ralph/scripts/run_loop.sh --dry-run           # 프롬프트만 출력, git/lock 변경 없음
#   ./ralph/scripts/run_loop.sh --count 5           # 최대 5 사이클 연속 실행
#   ./ralph/scripts/run_loop.sh T001 --no-reserve   # orchestrator가 이미 lock을 잡은 경우
...
if [ -f "$ROOT/ralph/scripts/lib/session_events.sh" ]; then
  # shellcheck source=./ralph/scripts/lib/session_events.sh
  . "$ROOT/ralph/scripts/lib/session_events.sh"
else
...
_op_dirty_lines() {
  git status --porcelain 2>/dev/null | awk '
    function is_op(p) {
      ...
      if (p == "ralph/docs/runbook.md")   return 1
      if (p == "README.md")         return 1
      if (p ~ /^ralph\/scripts\//) return 1
      if (p ~ /^ralph\/skills\//)  return 1
      if (p ~ /^ralph\/tests\//)   return 1
...
pick_next_ticket_path() {
  ...
  if [ "$SAFE_ONLY" = "1" ]; then
    out=$(./ralph/scripts/pick_next_ticket.sh --safe-only) || rc=$?
  else
    out=$(./ralph/scripts/pick_next_ticket.sh) || rc=$?
  fi
...
# 리뷰 2차 P1-7: 승인 마커 판정 단일 소스 — ralph/mission-control/approval.mjs.
...
validate_safe_false_approval() {
  ...
  local validator="$ROOT/ralph/mission-control/approval.mjs"

  if ! command -v node >/dev/null 2>&1 || [ ! -f "$validator" ]; then
    echo "❌ ${id}는 safe:false인데 승인 검증기(node + ralph/mission-control/approval.mjs)를 사용할 수 없습니다."
...
EOF
  ...
  # 리뷰 6차 P1: persona는 ralph/skills/<persona>.md 경로에 그대로 들어간다 — `../` 등
  # 경로 조작으로 ralph/skills/ 밖 파일이 페르소나로 로드되던 우회를 문자 화이트리스트로 차단.
  ...
  local skill_file="ralph/skills/${persona}.md"
  if [ ! -f "$skill_file" ]; then
    echo "❌ persona '$persona'에 대응하는 $skill_file 없음. 티켓을 다시 확인하세요."
    add_failure "${id:-unknown}" "unknown-persona" "${i:-0}" "persona='$persona'"
    return 12
  fi
  # 리뷰 7차 P1: 문자 화이트리스트는 symlink 우회(ralph/skills/linked.md → 외부 파일)를 못
  # 막는다 — skill 파일이 symlink면 거부해 ralph/skills/ 경계를 실제로 강제한다.
  if [ -h "$skill_file" ]; then
    echo "❌ $skill_file 은 symlink입니다 — ralph/skills/ 경계 밖 파일을 페르소나로 로드할 수 없습니다 (fail-closed)."
    add_failure "${id:-unknown}" "unknown-persona" "${i:-0}" "persona='$persona' (symlink)"
    return 12
  fi
  # 리뷰 8차 P1: ralph/skills/ 디렉터리 자체가 symlink인 우회도 물리 경로 대조로 차단.
  local sdir_real
  sdir_real="$(cd "$(dirname "$skill_file")" && pwd -P)" || sdir_real=""
  if [ "$sdir_real" != "$(pwd -P)/ralph/skills" ]; then
    echo "❌ skills 물리 경로('${sdir_real}')가 canonical ROOT/ralph/skills가 아닙니다 — symlink 여부를 확인하세요 (fail-closed)."
...
## 참고 (수정 금지)
- $skill_file
- ralph/docs/runbook.md (특히 §3 실패 처리, §4 보안 3요소)
- docs/master-spec.md  ← planner만 수정 가능

## 작업 지시
1. $skill_file 의 의무 절차를 단계 번호 그대로 수행.
2. 변경 후 \`./ralph/scripts/run_checks.sh\` 가 0 exit 이어야 합니다.
...
EOF
  ...
  (
    CLAUDE_TIMEOUT_SECONDS="$timeout_seconds" \
      ...
      ./ralph/scripts/run_headless.sh "$prompt" "$ROOT" 2>&1 | tee -a "$headless_log"
    exit "${PIPESTATUS[0]}"
  ) &
  HEADLESS_PID=$!
  set +m
  ...
  # ────────── 8. Verify ──────────
  echo "🔍 검증..."
  if ! ./ralph/scripts/run_checks.sh; then
...
EOF
    ...
    (
      CLAUDE_TIMEOUT_SECONDS="$timeout_seconds" \
        RALPH_SESSION_META_FILE="$reservation_meta" \
        RALPH_STATE_ROOT="$SESSION_STATE_ROOT" \
        ./ralph/scripts/run_headless.sh "$reprompt_prompt" "$ROOT" 2>&1 | tee -a "$headless_log"
      exit "${PIPESTATUS[0]}"
    ) &
    HEADLESS_PID=$!
    set +m
