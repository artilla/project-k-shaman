#!/usr/bin/env bats
# ralph/tests/rollback-guards.bats — 리뷰 2차 P1-10: rollback.sh 파괴 방지 가드 회귀 테스트.
#
...
_make_repo() {
  local d="$1"
  mkdir -p "$d/ralph/scripts"
  cp "$REPO_ROOT/ralph/scripts/rollback.sh" "$d/ralph/scripts/rollback.sh"
  chmod +x "$d/ralph/scripts/rollback.sh"
...
@test "R1: dirty tracked files -> refused exit 3 (protects work from reset --hard)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"미커밋 변경"* ]]
  # 파일이 파괴되지 않았어야 한다
...
@test "R2: main worktree non-interactive without --yes -> refused exit 3 (fail-closed)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/rollback.sh T200 < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"--yes"* ]]
  # 롤백되지 않았어야 한다
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"HEAD가 이동"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin2:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"revert 실패"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin3:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"revert 실패"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin4:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"태그가 그 사이 변경"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin5:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"CAS 실패"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin6:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin7:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin11:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"검증에 실패"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin12:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
...
WRAP
  ...
  local t0 t1
  t0="$(date +%s)"
  run bash -c 'cd "$1" && PATH="$1/gbin8:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  t1="$(date +%s)"
  [ "$status" -eq 130 ]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin9:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin10:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
  [[ "$output" == *"예상 밖 커밋"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin14:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
  [[ "$output" == *"checkout branch가 바뀌었습니다"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin15:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
...
@test "R31: pre-existing recovery ref is never clobbered nor deleted - evidence preserved regardless of ancestry" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo2"
  [ "$status" -eq 0 ]
  [[ "$output" == *"rolled back T200 by revert"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin16:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin17:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" == *"rev-list) 실패"* ]]
  [ "$(git -C "$repo" rev-parse HEAD)" = "$tip" ]
  grep -q "^v2$" "$repo/file.txt"
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin18:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
  [[ "$output" == *"예상 밖 커밋"* ]]
  # 복구 참조가 남는다
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/gbin19:$PATH" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo"
  [ "$status" -eq 3 ]
  [[ "$output" != *"restored"* ]]
...
WRAP
  ...
  (cd "$repo" && exec env PATH="$repo/gbin20:$PATH" ./ralph/scripts/rollback.sh T200 --yes) \
    > "$repo/.r36.log" 2>&1 &
  local bg=$! i=0 rc=0
...
WRAP
  ...
  (cd "$repo" && exec env PATH="$TEST_BASE/r37bin:$PATH" ./ralph/scripts/rollback.sh T200 --yes) \
...
@test "R38: TERM in the reset success-report window is honored - lock released, rc=130, no KILL needed" {
  ...
  local filler=$!
  sleep 0.5
  ( cd "$repo" && exec ./ralph/scripts/rollback.sh T200 > "$TEST_BASE/out.p" 2> "$TEST_BASE/r38.err" < /dev/null ) &
  local rpid=$!
...
SHIM
  ...
  run bash -c 'cd "$1" && PATH="$2:$PATH" TMPDIR="$3" ./ralph/scripts/rollback.sh T200 --yes < /dev/null' _ "$repo" "$TEST_BASE/r39bin" "$TEST_BASE/r39lnk"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
...
@test "R42: TERM right AFTER the reset success line is read is not lost as rc=0" {
  ...
  for attempt in 1 2 3 4 5; do
    ...
    ( cd "$repo" && exec ./ralph/scripts/rollback.sh T200 > "$TEST_BASE/r42.$attempt.p" 2> "$TEST_BASE/r42.$attempt.err" < /dev/null ) &
    rpid=$!
    line=""
...
MOCK
  ...
  run bash -c "cd '$repo' && PATH='$TEST_BASE/r44bin:'\"\$PATH\" exec ./ralph/scripts/rollback.sh T200 --yes"
  [ "$status" -eq 3 ]
  [[ "$output" != *"rolled back"* ]]
  ...
  local _t0=$SECONDS
  run bash -c 'cd "$1" && _TW_LOCK="state/ticket_write.lock.d" && source "$2/ralph/scripts/lib/write_lock.sh" && _acquire_write_lock && _release_write_lock' _ "$repo" "$REPO_ROOT"
  [ "$status" -eq 0 ]
  [ $((SECONDS - _t0)) -le 2 ]
...
@test "R43: TERM right AFTER the revert success line - no rc=0 disguise; lock released or immediately reclaimable (diagnostics best-effort)" {
  ...
  for attempt in 1 2 3 4 5; do
    ...
    ( cd "$repo" && exec ./ralph/scripts/rollback.sh T200 --yes > "$TEST_BASE/r43.$attempt.p" 2> "$TEST_BASE/r43.$attempt.err" < /dev/null ) &
    rpid=$!
    line=""
