// mission-control/ui.mjs — render helpers with CLI tooltip contract.
...
export function renderSessionInterventionBar(session = {}, { isLocalhost = true } = {}) {
  ...
  const actionButton = (action, label, extra = {}) => renderWriteButton({
    label,
    ariaLabel: `${id} ${action}`,
    cliCommand: `./scripts/session_ctl.sh ${action} ${id}`,
  ...
  const pause = canSignalClock
    ...
    : disabledSessionButton({
        ...
        cliCommand: `./scripts/session_ctl.sh pause ${id}`,
        reason: signalReason,
      });
  const resume = canSignalClock
    ...
    : disabledSessionButton({
        ...
        cliCommand: `./scripts/session_ctl.sh resume ${id}`,
        reason: signalReason,
      });
  const redirect = renderWriteButton({
    label: '지시 후 재시작',
    ariaLabel: `${id} redirect with instruction`,
    cliCommand: `./scripts/session_ctl.sh redirect ${id} "<instruction>"`,
    execCommand: 'session_ctl',
    ticketId: id,
  ...
  const abort = renderWriteButton({
    label: '⏹',
    ariaLabel: `${id} abort`,
    cliCommand: `./scripts/session_ctl.sh abort ${id}`,
    execCommand: 'session_ctl',
    ticketId: id,
...
export function renderRespondButton(id) {
  ...
  return renderWriteButton({
    ...
    cliCommand: `./scripts/session_ctl.sh redirect ${tid} "<instruction>"`,
    execCommand: 'session_ctl',
    ticketId: tid,
...
function renderDeviceRow(device) {
  ...
  if (device.active) {
    actions = `<span class="mc-rename"><input type="text" class="mc-rename-input" data-rename-input data-device-id="${escapeHtml(id)}" value="${escapeHtml(label)}" maxlength="64" aria-label="기기 라벨"></span>`
      + pairCommandButton({ label: '이름변경', cliCommand: `./scripts/pair.sh rename ${id} <new-label>`, action: 'rename', deviceId: id, className: 'mc-device-rename' })
      + pairCommandButton({ label: '폐기', cliCommand: `./scripts/pair.sh revoke ${id}`, action: 'revoke', deviceId: id, danger: true, className: 'mc-device-revoke' });
  }
...
export function renderPairingMain({ devices = [], pairingBase = '', limit = null } = {}) {
  ...
  const startButton = pairCommandButton({
    label: '페어링 시작',
    cliCommand: './scripts/pair.sh start',
    action: 'start',
  ...
  <section class="mc-pairing-panel" aria-labelledby="mc-pairing-title">
    ...
    <div class="mc-pairing-output" hidden data-pairing-output>
      ...
      <div class="mc-pairing-code">
        ...
        ${pairCommandButton({ label: '복사', cliCommand: './scripts/pair.sh start', action: 'copy', className: 'mc-pairing-copy' })}
