#!/usr/bin/env bats
# ralph/tests/reprompt-once.bats — T308 미커밋 WIP 1회 재프롬프트 회귀 테스트.
#
# 확인 사항:
...
_run_loop() {
  run env \
    ...
    /bin/bash "$TEST_HOME/ralph/scripts/run_loop.sh" T103-reprompt-test
}
...
@test "reprompt leaving detached background descendants -> reaped, result not trusted" {
  local counter="$WORKSPACE_ROOT/invocation-count"
  rm -f "$counter"
  cat > "$TEST_HOME/ralph/scripts/run_headless.sh" <<EOF2
#!/usr/bin/env bash
cd "\${2:-\$(pwd)}"
n=0
[ -f "$counter" ] && n=\$(cat "$counter")
