#!/usr/bin/env node
// ralph/mission-control/server.mjs
// ADR-0024: localhost-only, stateless, zero npm dependencies
//
// Usage:
//   node ralph/mission-control/server.mjs [--port <n>] [--root <path>] [--private-path <iface>]
...
const SKILLS_DIR     = join(ROOT, 'ralph', 'skills');
...
function readModelFingerprint() {
  const parts = [];
  // 리뷰 12차 P2: ralph/skills/ 변경(생성 포함)이 모델(skill_unavailable)에 반영되도록
  // fingerprint에 포함 — 시작 시 ralph/skills/가 없어 watcher 미등록이어도 다음 요청에서 재스캔.
  parts.push(`skills:${statFingerprint(SKILLS_DIR)}`);
        ...
        try {
          ...
          const ticket = {
            ...
            skill_unavailable: (() => {
              ...
              try {
                // 리뷰 11차 P2: ralph/skills/ 조상 경계까지 realpath 대조 (run_loop pwd -P 판정과 일치).
                if (realpathSync(SKILLS_DIR) !== join(realpathSync(ROOT), 'ralph', 'skills')) return true;
                const st = lstatSync(join(SKILLS_DIR, `${persona}.md`));
...
function renderTicketCard(ticket, doneIds, nowMs, isLocalhost = true, failCount = 0, ticketsById = {}, depIndex = null) {
  ...
  const runButton = isLocalhost && status === 'open' && !blocked && cardWritable
    ? renderWriteButton({
        label: 'Run',
        cliCommand: `./ralph/scripts/run_loop.sh ${ticket.id}`,
...
function renderTicketBodyEditor(ticket, isLocalhost, status) {
  ...
  const cli = `./ralph/scripts/ticket_body.sh set ${ticket.id} < body.md`;
  ...
  const aiCli = `./ralph/scripts/ai_draft.sh ticket-body ${ticket.id}`;
...
function renderTicketLifecycleControls(ticket, isLocalhost, status) {
  ...
  const cli = `./ralph/scripts/ticket_lifecycle.sh ${verb} ${ticket.id}`;
...
function renderTicketEditRow(ticket, isLocalhost, status) {
  ...
  return `<div class="mc-card-edit" data-ticket-edit-row data-ticket-id="${escapeHtml(ticket.id)}" data-cur-priority="${escapeHtml(cur)}" data-cur-labels="${escapeHtml(curLabels)}">
    ...
    <button type="button" class="mc-write mc-write--compact" data-ticket-edit-save title="$ ./ralph/scripts/ticket_edit.sh set-priority|set-labels ${escapeHtml(ticket.id)}" aria-description="$ ./ralph/scripts/ticket_edit.sh set-priority|set-labels ${escapeHtml(ticket.id)}">메타 저장</button>
  </div>`;
}

// ── Library ⑤ (ADR-0044): 읽기 전용 Playbook/Knowledge 브라우저 ─
// ralph/skills/*.md 페르소나 파서. parseFrontmatter는 인라인 배열만 다루므로 여기서는
...
function renderLibraryPage({ isLocalhost = true } = {}) {
  ...
  const cards = skills.length
    ...
    : '<p class="mc-empty">ralph/skills/ 없음</p>';
...
function renderSpecMasterEdit(isLocalhost, specMd) {
  if (!isLocalhost) return '';
  const cli = `./ralph/scripts/spec_edit.sh set --reason <reason> < spec.md`;
  return `<section class="mc-panel mc-specedit" aria-label="master-spec 편집" data-specedit>
    ...
    <details class="mc-specedit__body">
      ...
      <div class="mc-body-edit__actions">
        ...
        <button type="button" class="mc-write mc-write--compact mc-write--ai" data-spec-ai-draft title="$ ./ralph/scripts/ai_draft.sh master-spec" aria-description="$ ./ralph/scripts/ai_draft.sh master-spec">AI 초안 제안</button>
      </div>
      <div class="mc-ai-draft" data-ai-draft-out hidden>
    ...
    <details class="mc-newproj__body">
      ...
      <div class="np-transparency">
        <div>
          ...
          <ul><li>ralph/scripts/ · ralph/skills/ · ralph/mission-control/</li><li>docs/tickets/TEMPLATE.md · runbook.md</li><li>.gitignore · run_checks.local.sh</li><li>master-spec/README 스텁 · smoke 테스트</li></ul>
        </div>
        <div>
...
const LAUNCHPAD_HELP = [
  ...
  ['stack', '새 프로젝트에서 "통과(GREEN)"의 기준을 정합니다. node를 고르면 npm test 같은 검사가 자동 세팅되고, AI 루프는 이 검사가 통과해야만 작업을 완료로 인정합니다. "나중에"를 골라도 되지만, 검사를 채우기 전까지는 코드가 깨져도 통과처럼 보일 수 있어요. 언제든 ralph/scripts/run_checks.local.sh에서 바꿀 수 있습니다.'],
...
function renderHarnessNextSteps(prefix) {
  ...
  return `<details class="mc-next-steps" data-${p}-next hidden>
    ...
    <ol>
      ...
      <li><code>$EDITOR ralph/scripts/run_checks.local.sh</code> — 검증 스택이 none이면 검증 명령 채우기</li>
      <li><code>cp docs/tickets/TEMPLATE.md docs/tickets/T001-first.md</code> — 첫 티켓 작성</li>
      <li><code>./ralph/scripts/run_loop.sh T001-first --dry-run</code> — 프롬프트 미리보기</li>
      <li><code>./ralph/scripts/mission_control.sh start</code> — 이 프로젝트용 Mission Control 열기
    ...
    <section class="mc-panel mc-brownfield np-gate" data-bf hidden aria-label="기존 폴더 검사 결과">
      ...
      <div data-bf-result hidden>
        ...
        <div class="mc-brownfield__dirty" data-bf-dirty hidden>커밋되지 않은 변경이 있습니다. <strong>커밋이 이 작업의 유일한 복구 수단</strong>입니다 — 먼저 커밋한 뒤 다시 확인하세요.
          ...
          <button type="button" class="mc-write mc-write--compact mc-write--danger" data-bf-commit aria-description="$ ./ralph/scripts/init_new_project.sh --snapshot-commit <target> (add -A + 고정 wip 커밋 · push 없음 · 복구: git reset --soft HEAD~1)">커밋하고 다시 확인</button>
...
(() => {
  ...
  const cliFor = (state, target) => {
    ...
    if (state === 'occupied') return './ralph/scripts/init_new_project.sh --diff-manifest ' + target;
    ...
    return './ralph/scripts/init_new_project.sh ' + target + ' --stack ' + stack + (nm ? ' --name ' + nm : '') + (onb ? ' --with-onboarding' : '') + (glo ? ' --with-glossary' : '');
  };
  const paintPf = (state) => {
    pfState = state;
    lp.dataset.pf = state || '';
...
function renderDocEditPanel(isLocalhost) {
  ...
  const docs = [
    ...
    { key: 'skill:implementer', label: 'ralph/skills/implementer.md', path: join(SKILLS_DIR, 'implementer.md') },
    { key: 'skill:planner', label: 'ralph/skills/planner.md', path: join(SKILLS_DIR, 'planner.md') },
    { key: 'skill:reviewer', label: 'ralph/skills/reviewer.md', path: join(SKILLS_DIR, 'reviewer.md') },
    { key: 'skill:security-reviewer', label: 'ralph/skills/security-reviewer.md', path: join(SKILLS_DIR, 'security-reviewer.md') },
  ];
  const rows = docs.map(d => {
    let content = '';
    try { content = readFileSync(d.path, 'utf8'); } catch { content = ''; }
    const cli = `./ralph/scripts/doc_edit.sh set ${d.key} < doc.md`;
    ...
    const aiCli = `./ralph/scripts/ai_draft.sh doc ${d.key}`;
...
function renderRateConfigPanel(isLocalhost, ins) {
  ...
  const cli = './ralph/scripts/rate_config.sh set --in <X> --out <Y> [--cache-read <A>] [--cache-creation <B>]';
...
function computeSnapshotInventory() {
  ...
  scan(join(ROOT, 'docs'), 'docs', rel => rel === 'docs/master-spec.md' || (/^docs\/[^/]+\.md$/.test(rel)));
  scan(join(ROOT, 'ralph', 'docs'), 'ralph/docs', rel => rel === 'ralph/docs/runbook.md');
  // persona skills only.
  scan(SKILLS_DIR, 'ralph/skills', rel => ['ralph/skills/implementer.md', 'ralph/skills/planner.md', 'ralph/skills/reviewer.md', 'ralph/skills/security-reviewer.md'].includes(rel));
...
function renderAutonomyPage({ isLocalhost = true } = {}) {
  ...
  const controls = isLocalhost
    ...
    <div class="mc-mode-control__buttons">
      <button type="button" class="mc-write mc-write--compact" title="$ ./ralph/scripts/set_mode.sh co-pilot" aria-description="$ ./ralph/scripts/set_mode.sh co-pilot" data-set-mode="co-pilot">Co-pilot로 — safe:true 자동, safe:false 승인 대기</button>
      <button type="button" class="mc-write mc-write--compact" title="$ ./ralph/scripts/set_mode.sh suggest" aria-description="$ ./ralph/scripts/set_mode.sh suggest" data-set-mode="suggest">Suggest로 (조이기) — 실행 전 확인(dry-run)</button>
    </div>
  ...
  const grantControls = isLocalhost
    ? `<div class="mc-grant__controls">
      <button type="button" class="mc-write mc-write--compact" title="$ ./ralph/scripts/autopilot_grant.sh issue --budget 1 --expiry-min 30" aria-description="$ ./ralph/scripts/autopilot_grant.sh issue --budget 1 --expiry-min 30" data-autopilot-grant data-budget="1" data-expiry-min="30">무인 연속 운영 인가 (budget 1 · 30분)</button>
      ${isActive ? '<button type="button" class="mc-write mc-write--compact mc-write--danger" title="$ ./ralph/scripts/autopilot_grant.sh revoke" aria-description="$ ./ralph/scripts/autopilot_grant.sh revoke" data-autopilot-revoke>철회 (즉시 중단)</button>' : ''}
    </div>
...
(function(){
  ...
  <p class="mc-cli-note">$ ./ralph/scripts/run_loop.sh T&lt;id&gt; — 모든 버튼에는 동등한 CLI 명령이 툴팁으로 표시됩니다. UI가 죽어도 CLI로 완전 동작.</p>
...
function renderNewTicketForm(isLocalhost) {
  ...
  return `<details class="mc-newticket" data-newticket>
    ...
    <button type="button" class="mc-write mc-write--compact" data-nt-create title="$ ./ralph/scripts/new_ticket.sh create --title <title>" aria-description="$ ./ralph/scripts/new_ticket.sh create --title <title>">티켓 생성</button>
  </details>`;
}
...
function recoveryCommand(downside) {
  const found = String(downside || '').split('\n').map(s => s.trim())
    .find(s => /^(git\s+(revert|reset)|down|npm |node |\.\/ralph\/scripts\/|\.\/scripts\/)/.test(s));
...
function lowRiskDocsOnly(ticket) {
  ...
  if (/\b(ralph\/(mission-control|scripts|tests)\/|scripts\/|tests\/|state\/)/.test(scope)) return false;
...
function renderApprovalCard(ticket, failures, traceRuns = null) {
  ...
  const approveCmd = `./ralph/scripts/approve.sh ${ticket.id}`;
  const rejectCmd = `./ralph/scripts/approve.sh ${ticket.id} --reject "&lt;reason&gt;"`;
...
function execPlan(payload) {
  ...
  if (command === 'set_mode') {
    ...
    return {
      script: 'set_mode.sh',
      args: [mode],
      cliCommand: `./ralph/scripts/set_mode.sh ${mode}`,
    };
  }
  ...
  if (command === 'autopilot_grant') {
    ...
    return {
      ...
      cliCommand: `./ralph/scripts/autopilot_grant.sh issue --budget ${budget} --expiry-min ${expiryMin}`,
    };
  }
  ...
  if (command === 'autopilot_revoke') {
    return {
      script: 'autopilot_grant.sh',
      args: ['revoke'],
      cliCommand: `./ralph/scripts/autopilot_grant.sh revoke`,
    };
  }
  ...
  if (command === 'new_ticket') {
    ...
    return {
      ...
      args,
      stdin: body,
      cliCommand: `./ralph/scripts/new_ticket.sh create --title <title> --priority ${priority} --persona ${persona} < body.md`,
    };
  }
  ...
  if (command === 'doc_edit') {
    ...
    return {
      ...
      args,
      stdin: content,
      cliCommand: `./ralph/scripts/doc_edit.sh set ${docKey}${snapshot ? ' --snapshot' : ''} < doc.md`,
    };
  }
  ...
  if (command === 'spec_edit') {
    ...
    return {
      ...
      stdin: content,
      cliCommand: `./ralph/scripts/spec_edit.sh set --reason <reason> < spec.md`,
    };
  }
  ...
  if (command === 'ai_draft') {
    ...
    if (target === 'ticket-body') {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/ai_draft.sh ticket-body ${ticketId}`,
      };
    }
    ...
    if (target === 'doc') {
      ...
      return {
        script: 'ai_draft.sh',
        args: ['doc', docKey],
        cliCommand: `./ralph/scripts/ai_draft.sh doc ${docKey}`,
      };
    }
    ...
    if (target === 'master-spec') {
      return {
        script: 'ai_draft.sh',
        args: ['master-spec'],
        cliCommand: `./ralph/scripts/ai_draft.sh master-spec`,
      };
    }
    ...
    if (target === 'interview') {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/ai_draft.sh interview ${turn}`,
      };
    }
  ...
  if (command === 'rate_config') {
    ...
    return { script: 'rate_config.sh', args, cliCommand: `./ralph/scripts/rate_config.sh ${args.join(' ')}` };
  }
  ...
  if (command === 'prune_preview') {
    ...
    return {
      ...
      args,
      cliCommand: `./ralph/scripts/prune_preview.sh ${args.join(' ')}`,
    };
  }
  ...
  if (command === 'init_new_project') {
    ...
    if (payload?.snapshotCommit === true) {
      return {
        ...
        cliCommand: `./ralph/scripts/init_new_project.sh --snapshot-commit ${targetPath}`,
      };
    }
    ...
    if (payload?.scan === true) {
      return {
        ...
        cliCommand: `./ralph/scripts/init_new_project.sh --diff-manifest ${targetPath}`,
      };
    }
    ...
    return {
      script: 'init_new_project.sh',
      args,
      cliCommand: `./ralph/scripts/init_new_project.sh ${targetPath} --stack ${stack}${name ? ' --name ' + name : ''}${force ? ' --force' : ''}${withOnboarding ? ' --with-onboarding' : ''}${withGlossary ? ' --with-glossary' : ''}${dryRun ? ' --dry-run' : ''}`,
    };
  }
  ...
  //  - 스크립트가 하네스 마커(ralph/mission-control/server.mjs) 없는 폴더를 거부 —
  ...
  if (command === 'open_project_mc') {
    ...
    return {
      ...
      cliCommand: `./ralph/scripts/open_project_mc.sh ${targetPath}${port !== undefined ? ' --port ' + port : ''}`,
    };
  }
  ...
  if (command === 'ticket_edit') {
    ...
    if (action === 'set-priority') {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/ticket_edit.sh set-priority ${ticketId} ${priority}`,
      };
    }
    if (action === 'set-labels') {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/ticket_edit.sh set-labels ${ticketId} ${shellQuote(labels)}`,
      };
    }
  ...
  if (command === 'ticket_lifecycle') {
    ...
    return {
      ...
      cliCommand: `./ralph/scripts/ticket_lifecycle.sh ${action} ${ticketId}`,
    };
  }
  ...
  if (command === 'ticket_body') {
    ...
    return {
      ...
      args: ['set', ticketId],
      stdin: body,
      cliCommand: `./ralph/scripts/ticket_body.sh set ${ticketId} < body.md`,
    };
  }
  ...
  if (command === 'run_loop') {
    return {
      ...
      detach: true,
      cliCommand: `./ralph/scripts/run_loop.sh ${ticketId}`,
    };
  }
  ...
  if (command === 'approve') {
    ...
    if (rejectReason) {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/approve.sh ${ticketId} --reject ${shellQuote(rejectReason)}`,
      };
    }
    return {
      script: 'approve.sh',
      args: [ticketId],
      cliCommand: `./ralph/scripts/approve.sh ${ticketId}`,
    };
  }
  ...
  if (command === 'session_ctl') {
    ...
    if (action === 'redirect') {
      ...
      return {
        ...
        cliCommand: `./ralph/scripts/session_ctl.sh redirect ${ticketId} ${shellQuote(instruction)}`,
      };
    }
    ...
    return {
      script: 'session_ctl.sh',
      args: [action, ticketId],
      cliCommand: `./ralph/scripts/session_ctl.sh ${action} ${ticketId}`,
    };
  }
...
function runScript(plan) {
  ...
  if (plan.detach) {
    try {
      ...
      const child = spawn(join(ROOT, 'ralph', 'scripts', plan.script), plan.args, {
        cwd: ROOT,
