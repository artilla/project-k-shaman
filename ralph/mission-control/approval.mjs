// ralph/mission-control/approval.mjs
// 리뷰 2차 P1-7: safe:false 승인 마커 검증 단일 소스.
// Inbox(UI, inbox.mjs)와 실행기(ralph/scripts/run_loop.sh validate_safe_false_approval)가
...
//   node ralph/mission-control/approval.mjs <root> <TXXX>
...
if (isMainModule()) {
  ...
  if (!root || !id || !/^T\d+$/.test(id)) {
    console.error('usage: node ralph/mission-control/approval.mjs <root> <TXXX>');
    process.exit(2);
  }
