#!/usr/bin/env bash
# ralph/scripts/open_project_mc.sh — 새 프로젝트(타깃)의 Mission Control 시작 + URL 출력
# T303 (ADR-0207): 완료 화면 "다음 단계"를 웹에서 이어가기 위한 단일 실행 경로.
#
# 사용법:
#   ./ralph/scripts/open_project_mc.sh <target-dir> [--port <n>]
#
# 안전 규칙:
#   - 대상은 SOURCE(이 저장소) 밖이어야 한다 (init_new_project와 동일 불변).
#   - 대상에 하네스 마커(ralph/mission-control/server.mjs + ralph/scripts/mission_control.sh)가
...
usage() { echo "사용법: ./ralph/scripts/open_project_mc.sh <target-dir> [--port <n>]"; }
...
if [ ! -f "$TGT/ralph/mission-control/server.mjs" ]; then
  echo "오류: 하네스 마커 없음 — $TGT/ralph/mission-control/server.mjs 가 없습니다." >&2
  echo "      (T303 이전 하네스로 만든 프로젝트라면 하네스를 다시 적용하세요: ./ralph/scripts/init_new_project.sh --diff-manifest 로 확인 후 적용)" >&2
  exit 1
fi
if [ ! -f "$TGT/ralph/scripts/mission_control.sh" ]; then
  echo "오류: 하네스 마커 없음 — $TGT/ralph/scripts/mission_control.sh 가 없습니다." >&2; exit 1
fi
...
(
  ...
  PATH="$EXTRA_PATH:$PATH" nohup "$NODE_BIN" ralph/mission-control/server.mjs --port "$PORT" >> "$LOG_FILE" 2>&1 &
  echo $! > "$PID_FILE"
)
