...
BASE = Path(__file__).resolve().parents[1]   # fortune-engine
KIT = Path(__file__).parent
samples = json.loads((BASE/"fortune-samples.v1.json").read_text(encoding="utf-8"))["samples"]
...
for s in pick:
    ...
    ab.append({
        "seed": s["meta"]["seed_hash"], "topic": s["meta"]["topic"],
        "A_llm": llm_narr, "B_assembled": asm_narr,
        "pred": {
...
(KIT/"ab_scripts.json").write_text(json.dumps({"rate_slow":RATE_SLOW,"items":ab}, ensure_ascii=False, indent=2), encoding="utf-8")
...
L.append("대상: 주제별 1개씩 5개 샘플. A=현재 LLM 출력 narration, B=`narration_composer.py` 서버 조립.\n")
L.append("> 핵심: **TTS 오디오 길이/요금은 두 버전이 거의 같다**(아래 예측표). 비용 이득은 LLM **출력 토큰** 절감(별도 리포트)에서 나온다. 이 A/B는 **자연스러움(특히 scores 문장과 연결)** 이 유지되는지를 귀로 판정하기 위한 것이다.\n")
...
for x in ab:
    p = x["pred"]
    L.append(f"| {x['seed']} | {x['topic']} | {p['A_full_sec_slow']} | {p['B_full_sec_slow']} | {p['A_fresh_sec_slow']} | {p['B_fresh_sec_slow']} |")
# TTS 요금 메모
def cost(sec):
...
L.append("- **scores 세그먼트만 실질 차이**: A는 LLM이 문맥에 맞춰 쓴 문장, B는 최고/최저 점수 기반 템플릿. 청취 QA의 핵심 포인트.\n")
L.append("- 따라서 v1.1 결정은 사실상 \"scores 문장을 템플릿으로 대체해도 자연스러운가\"로 좁혀진다. 중간안: scores만 LLM이 짧은 한 문장(`scores_line`) 출력(출력 토큰 소폭 증가, 자연스러움 보존).\n")

(KIT/"ab-comparison.md").write_text("\n".join(L), encoding="utf-8")
print("wrote ab_scripts.json, ab-comparison.md")
print(f"A fresh avg {aA:.1f}s, B fresh avg {aB:.1f}s")
for x in ab:
    print(x["topic"], x["seed"], "A",x["pred"]["A_full_sec_slow"],"B",x["pred"]["B_full_sec_slow"])
