...
실행: python -m tools.analytics.measure_playback [--events-log <path>]
"""
...
_DEFAULT_EVENTS_LOG_PATH = _ROOT / "state" / "events" / "playback_events.jsonl"
...
def summarize(records: list) -> dict:
    ...
    return {
        ...
        "avgTextLatencyMs": (sum(text_latencies) / len(text_latencies))
        if text_latencies
        else None,
        "avgAudioLatencyMs": (sum(audio_latencies) / len(audio_latencies))
        if audio_latencies
        else None,
        "cacheHitRate": cache_hit_rate,
        "newSynthesisCount": len(synthesis_costs),
        "estimatedNewSynthesisCostUsd": round(sum(synthesis_costs), 5),
        "shareInitiatedRate": (share_initiated_count / eligible_sessions)
        if eligible_sessions
        else None,
        "shareCompletedRate": (share_completed_count / eligible_sessions)
        if eligible_sessions
        else None,
    }
...
def format_report(summary: dict) -> str:
    ...
    if summary["cacheHitRate"] is not None:
        verdict = (
            "PASS"
            if summary["cacheHitRate"] >= _CACHE_HIT_RATE_THRESHOLD
    ...
    if summary["sessions"]:
        ...
        verdict = (
            "PASS"
            if cost_per_session <= _COST_PER_SESSION_THRESHOLD_USD
    ...
    if summary["shareCompletedRate"] is not None:
        verdict = (
            "PASS"
            if summary["shareCompletedRate"] >= _SHARE_RATE_THRESHOLD
            else "BELOW threshold"
        )
        lines.append(
