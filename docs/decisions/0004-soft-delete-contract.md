...
**강제 범위 (정직한 한계 — 재리뷰 지적 반영)**: 스키마가 강제하는 것은 "삭제
경로가 child→users 순서를 따른다"는 것뿐이다. **전역 순서를 스키마가 강제하지는
않는다.** 특히 여러 child 테이블을 위 순서의 **역순**으로 갱신하는 writer는 다른
writer나 삭제와 여전히 deadlock을 만들 수 있다 — 이 부분은 **규약**이다:

> 여러 child 테이블/행을 갱신하는 writer는 `events → sessions → streaks →
> user_fortunes` 순서로, 각 테이블 안에서는 id 오름차순으로 잠근다.

규약 위반으로 발생하는 잔여 deadlock은 `app_soft_delete_user`의 bounded
retry(subtransaction rollback → 최대 5회)가 삭제 쪽에서 흡수하지만, writer 쪽은
서버의 deadlock victim이 될 수 있다. 완전한 강제가 필요해지면 별도 티켓에서
단일 진입 함수(모든 child 갱신을 감싸는 API)를 검토한다.

### 진입점 권한 경계 (8라운드 후속 P1 → 재리뷰 P1 #1~#4)

과거의 transaction-local GUC(`app.soft_delete_user`)는 일반 app role도 정확한
ID로 `set_config`할 수 있어 **증거가 아니었다** (실측: 위조 후 직접 UPDATE로
삭제 성공). 현재 경계는 네 겹이다:

1. **정의자 role**: `shaman_softdelete` (NOLOGIN). `app_soft_delete_user()`는 이
...
4. **search_path 고정 (#2)**: `SET search_path FROM CURRENT`는 pg_temp를 명시하지
   ...
   스크럽되지 않았다(실측). 정의자 컨텍스트에서 실행되는 모든 함수의 search_path를
   `pg_catalog, <schema>, pg_temp`로 **고정**한다 — pg_temp가 마지막이므로 임시
   객체가 permanent 객체를 가릴 수 없고, unqualified 참조는 항상 대상 스키마로
   해석된다.
