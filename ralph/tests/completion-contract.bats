#!/usr/bin/env bats
# ralph/tests/completion-contract.bats — 리뷰 2차 P1-8: run_loop 완료 판정 계약 회귀 테스트.
#
# rc=0(성공)으로 인정하려면:
#   (1) op 경로 clean
...
_make_home() {
  ...
  cp "$REPO_ROOT/ralph/scripts/run_loop.sh" "$home/ralph/scripts/run_loop.sh"
  chmod +x "$home/ralph/scripts/run_loop.sh"
...
state/reservations/
.ralph/
ralph/scripts/run_headless.sh
EOF
  # 참고: ralph/scripts/run_headless.sh는 각 테스트가 다시 쓰는 fake다. ignore하지 않으면
  # isolated worktree pre-flight의 `git clean -fd`가 untracked인 fake를 지워버린다.

  cat > "$home/ralph/scripts/run_checks.sh" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
  chmod +x "$home/ralph/scripts/run_checks.sh"

  echo "# implementer stub" > "$home/ralph/skills/implementer.md"
  echo "# Master Spec" > "$home/docs/master-spec.md"
  echo "# Runbook" > "$home/ralph/docs/runbook.md"
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T100' _ "$TEST_HOME"
  [ "$status" -eq 8 ]
...
@test "C6: main worktree - pre-existing user WIP survives, proper completion -> rc=0" {
  ...
  cat > "$main_home/ralph/scripts/run_headless.sh" <<EOF
${_fake_headless_prologue}
tmp=\$(mktemp)
awk '/^---\$/ { fm = !fm; print; next } fm && \$1 == "status:" { print "status: done"; next } { print }' "\$ticket" > "\$tmp"
mv "\$tmp" "\$ticket"
...
EOF
  chmod +x "$main_home/ralph/scripts/run_headless.sh"

  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T100' _ "$main_home"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && PATH="$1/bin:$PATH" ./ralph/scripts/run_loop.sh T100' _ "$main_home"
  [ "$status" -eq 13 ]
  [[ "$output" == *"fingerprint 수집 실패"* ]]
