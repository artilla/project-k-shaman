#!/usr/bin/env bats
# ralph/tests/check-scope-omission.bats — check_scope_omission.sh 회귀 테스트 (T050)
#
# 격리: 각 테스트는 mktemp -d 임시 디렉터리에서 독립 실행.
# git 의존 없이 --changed-files 옵션으로 완전 격리.

SCRIPT_PATH="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)/ralph/scripts/check_scope_omission.sh"
...
- [ ] `ralph/scripts/check_scope_omission.sh` 존재 + executable
EOF
  printf 'ralph/scripts/check_scope_omission.sh\n' > "$TEST_DIR/changed.txt"
...
- [ ] `ralph/scripts/foo.sh` 생성
EOF
  printf 'ralph/scripts/foo.sh.bak\n' > "$TEST_DIR/changed.txt"
  run "$SCRIPT_PATH" "$TEST_DIR/ticket.md" --changed-files "$TEST_DIR/changed.txt"
  [ "$status" -eq 1 ]
  [[ "$output" == *"ralph/scripts/foo.sh"* ]]
}
...
- `ralph/scripts/external.sh` 이 파일은 참조용 경로
