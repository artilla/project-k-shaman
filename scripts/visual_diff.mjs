...
//
// 사용:
//   node scripts/visual_diff.mjs                        # 기본: new-project 3개 상태
//   node scripts/visual_diff.mjs --gate 5               # 불일치율 5% 초과 시 실패
//   CHROME_BIN=/path/to/chrome node scripts/visual_diff.mjs
//   node scripts/visual_diff.mjs --live-url http://127.0.0.1:7474  # 떠있는 서버 사용
//
...
async function startServer(port) {
  const proc = spawn(process.execPath, [join(ROOT, 'mission-control', 'server.mjs'), '--port', String(port)],
