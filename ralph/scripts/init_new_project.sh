...
#        ./ralph/scripts/init_new_project.sh
...
#        ./ralph/scripts/init_new_project.sh <target-dir> [옵션]
#
# 철학: 이식 가능한 하네스(ralph/·TEMPLATE·.gitignore)만 복사하고,
...
#   --stack <s>         node | python | go | rust | none → ralph/scripts/run_checks.local.sh 자동 생성
...
if [ "${1:-}" = "--diff-manifest" ]; then
  ...
  for p in ralph/scripts ralph/skills ralph/mission-control ralph/docs/runbook.md docs/tickets/TEMPLATE.md docs/approvals/README.md .gitignore; do
    if [ -e "$DM_ABS/$p" ]; then
  ...
  for g in ralph/tests/smoke.bats ralph/scripts/run_checks.local.sh docs/master-spec.md README.md; do
...
# 클린 추출: ralph/ 하네스만. Ralph 누적 테스트는 통째로 복사하지 않는다 —
# Hephaestus 전용 Mission Control 테스트는 새 레포에서 통과하지 못하므로
# 빈 스켈레톤 + 통과하는 starter smoke 테스트(아래 [2/6])만 둔다.
HARNESS_PATHS=(
  "ralph/scripts" "ralph/skills" "ralph/mission-control" "ralph/docs/runbook.md"
  "docs/tickets/TEMPLATE.md" "docs/approvals/README.md" ".gitignore"
)
SKELETON_DIRS=(
  "docs/tickets/DONE" "docs/tickets/ARCHIVE" "docs/decisions" "docs/reviews"
  "ralph/tests" "state" "state/reservations" ".ralph/logs"
)
...
if [ "$DRY_RUN" -eq 0 ]; then
  chmod +x "$TARGET"/ralph/scripts/*.sh 2>/dev/null || true
  [ -d "$TARGET/ralph/scripts/lib" ] && chmod +x "$TARGET"/ralph/scripts/lib/*.sh 2>/dev/null || true
  # T303: mission-control은 서버 본체만 이식 — Hephaestus 유닛 테스트는 새 레포 소관이 아님
  rm -f "$TARGET/ralph/mission-control/"*.test.mjs 2>/dev/null || true
fi
...
step "ralph/tests/smoke.bats (starter 예시 — Hephaestus Ralph 테스트는 비복사)"
if [ "$DRY_RUN" -eq 0 ]; then
...
EOF
  ...
  cat > "$TARGET/ralph/tests/smoke.bats" <<'EOF'
#!/usr/bin/env bats
...
# ─────────────────────────────────────────────────────────
# 8. 검증 스택 → ralph/scripts/run_checks.local.sh
# ─────────────────────────────────────────────────────────
say ""
say "[3/6] 검증 스택 설정 (ralph/scripts/run_checks.local.sh)"
write_local_checks() {
  local body
  case "$STACK" in
...
echo "(아직 프로젝트별 검증 미설정 — run_checks.local.sh를 편집하세요)"' ;;
  ...
  [ "$DRY_RUN" -eq 1 ] && return 0
  cat > "$TARGET/ralph/scripts/run_checks.local.sh" <<EOF
#!/usr/bin/env bash
...
$body
EOF
  chmod +x "$TARGET/ralph/scripts/run_checks.local.sh" 2>/dev/null || true
}
write_local_checks
...
$EDITOR ralph/scripts/run_checks.local.sh    # 2) 검증 명령 확인/수정
./ralph/scripts/run_checks.sh                # 3) 0 exit 확인
cp docs/tickets/TEMPLATE.md docs/tickets/T001-first.md
$EDITOR docs/tickets/T001-first.md     # 4) 첫 티켓
./ralph/scripts/run_loop.sh T001-first --dry-run   # 5) 프롬프트 미리보기
```
...
if ( cd "$TARGET" && ./ralph/scripts/run_checks.sh >/dev/null 2>&1 ); then
  step "✓ run_checks.sh 0 exit (새 레포 검증 통과)"
else
  step "⚠ run_checks.sh 비-0 exit — 'cd $TARGET && ./ralph/scripts/run_checks.sh'로 확인"
fi
...
[ "$STACK" = "none" ] && echo '  $EDITOR ralph/scripts/run_checks.local.sh   # 검증 명령 채우기'
cat <<EOF
  cp docs/tickets/TEMPLATE.md docs/tickets/T001-first.md
  ./ralph/scripts/run_loop.sh T001-first --dry-run
