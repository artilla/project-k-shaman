#!/usr/bin/env bats
# tests/headless_diagnostics.bats — T102 진단 번들 보존 회귀 테스트.
#
# 확인 사항:
...
_run_loop() {
  run env \
    ...
    /bin/bash "$TEST_HOME/scripts/run_loop.sh" T102-diag-test
}
...
_install_real_headless_with_gtimeout_fixture() {
  cp "$REPO_ROOT/scripts/run_headless.sh" "$TEST_HOME/scripts/run_headless.sh"
  chmod +x "$TEST_HOME/scripts/run_headless.sh"
  cp "$REPO_ROOT/scripts/lib/session_events.sh" "$TEST_HOME/scripts/lib/session_events.sh"
  git -C "$TEST_HOME" add "$TEST_HOME/scripts/run_headless.sh" "$TEST_HOME/scripts/lib/session_events.sh"
