#!/usr/bin/env bats
# ralph/tests/run_headless_stream_heartbeat.bats — T305 스트림 하트비트 회귀 테스트.
#
...
# stream-json --verbose로 실행하고 ralph/scripts/lib/heartbeat_capture.mjs를 거치도록
...
setup() {
  ...
  WORK="$TEST_HOME/work"
  mkdir -p "$TEST_BIN" "$WORK" "$WORK/ralph/scripts/lib" "$TEST_HOME/state/reservations/T890.d"
...
@test "heartbeat_capture.mjs: assistant text passes through, non-text events become [hb] lines, malformed JSON passes through" {
  run node "$REPO_ROOT/ralph/scripts/lib/heartbeat_capture.mjs" <<'EOF'
