...
#
#   snapshot_ls.sh docs/runbook.md   → inventory for one base
...
shopt -s nullglob
bases=()
for f in docs/*.v[0-9]*.md skills/*.v[0-9]*.md; do
  ...
  doc="${b}.md"
  case "$doc" in
    docs/master-spec.md|docs/runbook.md|docs/*.md) ok=1 ;;
    skills/implementer.md|skills/planner.md|skills/reviewer.md|skills/security-reviewer.md) ok=1 ;;
    *) ok=0 ;;
  esac
  [ "$ok" = 1 ] || continue
  bases+=("$b")
