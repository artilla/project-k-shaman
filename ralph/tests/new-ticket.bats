#!/usr/bin/env bats
# ralph/tests/new-ticket.bats — new_ticket writer의 경로·안전 기본 계약.

setup() {
  REPO_ROOT="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)"
  TEST_HOME="$(mktemp -d)"
  mkdir -p "$TEST_HOME/ralph/scripts" "$TEST_HOME/docs/tickets/DONE"
  cp "$REPO_ROOT/ralph/scripts/new_ticket.sh" "$TEST_HOME/ralph/scripts/new_ticket.sh"
  chmod +x "$TEST_HOME/ralph/scripts/new_ticket.sh"

  git -C "$TEST_HOME" init -q
  git -C "$TEST_HOME" config user.email "new-ticket@test"
  git -C "$TEST_HOME" config user.name "new-ticket-test"
  NEW_TICKET="$TEST_HOME/ralph/scripts/new_ticket.sh"
}
