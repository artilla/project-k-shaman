...
모바일 PWA의 **화면 정보구조(IA)와 화면별 와이어프레임 수준 명세**가 한 문서로 서서, 이후 화면 구현 implementer 티켓들이 이 골격을 기준으로 붙는다. (Sprint 1: "모바일 화면 IA 작성" — docs/planning/Plan.md §12·§15.3)

## 2. 변경 범위 (Scope)

**포함**
- 신규 문서 `docs/ux/screen-ia.md` 작성 — 화면 인벤토리·전이·화면별 요소/상태·이벤트 매핑.
- 기존 결정에 grounded: `docs/planning/Plan.md` §4(핵심 흐름)·§5(Frontend 백로그)·§5(Analytics 이벤트), `docs/planning/today-shindang-service-plan-v3.md` §11(재생 UX·autoplay 금지), §12(개인정보 UX), 캐릭터 시트 §6(아바타 상태).
...
- [ ] **화면 인벤토리 + 전이도**: 온보딩 → 무당 선택(홍연 고정) → 입력(닉네임·생년월일·출생시간) → 주제 선택 → 캐릭터 stage → 결과 카드 → 공유. (docs/planning/Plan.md §4 9단계와 1:1 매핑)
...
- [ ] **분석 이벤트 매핑**: 화면/액션 → 이벤트(`fortune_start`·`character_select`·`tts_play_start/complete`·`share_card_create` 등, docs/planning/Plan.md §5).
