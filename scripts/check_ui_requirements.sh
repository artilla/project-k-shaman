...
usage() {
  cat <<'EOF'
usage: scripts/check_ui_requirements.sh [--html FILE ...] [--css FILE ...] [--manifest FILE ...] [--sw FILE ...] [--ui-js FILE ...]

Without --html/--css, starts mission-control/server.mjs on localhost and checks
rendered /, /inbox, /sessions, /ui.css, /manifest.webmanifest, /sw.js,
mission-control/ui.mjs, plus docs/assets/*.html.
EOF
}
...
if [ "${#HTML_FILES[@]}" -eq 0 ] && [ "${#CSS_FILES[@]}" -eq 0 ] && [ "${#MANIFEST_FILES[@]}" -eq 0 ] && [ "${#SW_FILES[@]}" -eq 0 ] && [ "${#UI_JS_FILES[@]}" -eq 0 ]; then
  ...
  node "$ROOT/mission-control/server.mjs" --root "${MC_ROOT:-$ROOT}" --port "$PORT" >"$TEMP_DIR/server.log" 2>&1 &
  SERVER_PID=$!
  sleep 0.6
  ...
  UI_JS_FILES+=("$ROOT/mission-control/ui.mjs")
