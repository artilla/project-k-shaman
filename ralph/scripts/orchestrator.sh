...
#
# 사용:
#   ./ralph/scripts/orchestrator.sh                  # 동시 워커 1, 한 라운드
#   ./ralph/scripts/orchestrator.sh --max 3          # 동시 워커 최대 3
#   ./ralph/scripts/orchestrator.sh --watch          # 새 티켓 들어오면 계속
...
spawn_worker() {
  ...
  # `cd $wt` 후 `./ralph/scripts/run_loop.sh`를 호출하므로 worker의 ROOT 결정 로직(pwd)도 백업으로 같은 경로를 잡는다.
  ...
  (
    cd "$worker_root"
    RALPH_ROOT="$worker_root" RALPH_STATE_ROOT="$ROOT" ./ralph/scripts/run_loop.sh "$id" --count 1 --no-reserve
  ) >"$logfile" 2>&1 &
...
run_round() {
  ...
  while [ "$active" -lt "$MAX_WORKERS" ]; do
    local next; next=$(./ralph/scripts/pick_next_ticket.sh)
    [ -z "$next" ] && break
...
if [ "$WATCH" = "1" ]; then
  ...
  if ! autopilot_grant_valid; then
    ...
    echo "   발급: ./ralph/scripts/autopilot_grant.sh issue --budget N --expiry-min M (또는 Mission Control localhost)."
