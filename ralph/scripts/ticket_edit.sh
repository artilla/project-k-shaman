...
#
# 사용:
#   ./ralph/scripts/ticket_edit.sh set-priority T123 P1
#   ./ralph/scripts/ticket_edit.sh set-labels   T123 "ui,autonomy,test"
set -euo pipefail
...
# shellcheck source=ralph/scripts/lib/write_lock.sh
. "$ROOT/ralph/scripts/lib/write_lock.sh"
...
# 리뷰 16차 P1: 편집 전체(기준 캡처 → 검증 → publish → 감사 커밋)를 단일
# 임계구역으로. 기준(EXPECT_INO/EXPECT_SHA)과 old 값을 lock "밖"에서 캡처하면,
# lock 대기 후 차례가 온 정당한 직렬 편집이 선행 writer의 publish를 identity/SHA
# 변경으로 오인해 거부된다 — 편집이 조용히 유실되는 lost update (T64 재현).
# 기준 캡처를 lock 획득 "후"로 옮기면 각 writer는 항상 직전 writer가 publish한
# 최신 상태 위에서 CAS를 수행한다.
if ! _acquire_write_lock; then
  echo "❌ write lock 획득 실패 — 편집하지 않습니다." >&2
  exit 4
fi

# 리뷰 11차 P1: 읽기 시점 identity 고정 — 이후 모든 쓰기는 같은 dev/inode여야 한다.
# (리뷰 16차: lock 아래에서 캡처 — 위 주석 참조)
...
commit_edit() {  # 단일 감사 커밋(가역). git 저장소가 아니면 파일만 갱신.
  ...
  if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    if ! git diff --cached --quiet -- "$file"; then
      # 리뷰 16차 P1: 무경로 commit은 index에 미리 staged된 "무관한" 변경까지
      # 이 감사 커밋에 흡수했다 (귀속 오염 + git revert 시 무관 변경 동반 파괴).
      # pathspec 커밋으로 이 티켓 파일만 커밋한다 (다른 staged 항목은 보존).
      git commit -m "$msg" -- "$file" >/dev/null
    fi
  fi
}
...
# 리뷰 16차: 최상위 lock 해제 (비정상 종료는 EXIT trap이 정리).
_release_write_lock
