...
usage:
  ...
  ralph/scripts/approve.sh --reject "reason" <TXXX>
EOF
}
...
if command -v node >/dev/null 2>&1 && [ -f "$ROOT/ralph/mission-control/approval.mjs" ]; then
  VALIDATION="$(node "$ROOT/ralph/mission-control/approval.mjs" "$ROOT" "$ID" 2>&1 || true)"
