...
`fortune_api_mock`이 `build_seed`(T011)로 결정적 키·`seed_signals`를 도출하도록 연결되어, **T010 mock과 T011 seed 규칙이 하나의 계약**으로 묶인다(캐시 키가 docs/planning/Plan.md §10 규칙을 따름).
...
- 이 통합으로 **캐시 키 = 실제 seed 규칙**(docs/planning/Plan.md §10). 실제 HMAC·LLM·TTS·HTTP는 여전히 후속/§3 hold.
