#!/usr/bin/env bats
# ralph/tests/auto-merge-execute.bats — auto_merge.sh --execute 회귀 테스트 (T060)
#
...
SCRIPT_PATH="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)/ralph/scripts/auto_merge.sh"
...
make_repo() {
  ...
  while [ "$i" -le "$num" ]; do
    if [ "$area" = "docs" ]; then
      ...
      printf '#!/bin/sh\necho hi\n' > "$d/ralph/scripts/tool-$i.sh"
      git -C "$d" add "ralph/scripts/tool-$i.sh"
    fi
...
@test "execute: code change NOT ELIGIBLE -> no merge exit 1 base unchanged" {
  local d="$TEST_HOME/repo"
  mkdir -p "$d"
  make_repo "$d" 1 scripts  # ralph/scripts/ 변경 → 조건 2 FAIL
