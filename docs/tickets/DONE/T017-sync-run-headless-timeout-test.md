...
## 1. 목표 (한 줄)
> `./ralph/scripts/run_checks.sh --fast`가 watchdog fallback 테스트에서 멈추지 않고 완주하게 한다 — 루프의 "통과(GREEN)" 기준 복구.

## 2. 변경 범위 (Scope)

**포함**
- `ralph/tests/run-headless-timeout.bats` 1개 파일을 업스트림(Hephaestus `0c9cfb7` 기준) 버전으로 교체.

**제외**
- `ralph/scripts/run_headless.sh` — 변경 금지 (업스트림과 이미 바이트 동일 확인).
- 그 외 tests/·제품 코드.

## 3. 원인 진단 (조사 완료 — 구현 전 재확인만)

2026-07-06 하네스 재적용(T303)으로 `ralph/scripts/`는 업스트림 최신과 동일해졌지만, 하네스는
...
재현: ProjectK-Shaman에서 `./ralph/scripts/run_checks.sh --fast` → "bash watchdog fallback" 계열
테스트에서 멈춤 (2026-07-06 운영자 재현).

## 4. 수용 기준
- [ ] `ralph/tests/run-headless-timeout.bats`가 업스트림 버전과 바이트 동일
- [ ] `bats ralph/tests/run-headless-timeout.bats` 전 케이스 통과(또는 명시적 skip) — 멈춤 없음
- [ ] `./ralph/scripts/run_checks.sh --fast` 완주 (0 exit)
