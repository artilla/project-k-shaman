...
#
# 사용:
#   ./scripts/verify_mobile_pairing.sh
#
# exit 0: 모든 사전점검 PASS
...
import { requiresTokenAuth, execScopeDecision, tokenAuthDecision } from '$ROOT/mission-control/token-auth.mjs';
...
# 실제 mission-control/에서 로드되고(cwd=ROOT), 데이터 디렉터리만 --root를 따른다.
...
mkdir -p "$SERVER_ROOT/docs/tickets/DONE" "$SERVER_ROOT/docs/approvals" \
         "$SERVER_ROOT/scripts" "$SERVER_ROOT/state/reservations"
cp "$ROOT/scripts/approve.sh" "$SERVER_ROOT/scripts/approve.sh"
chmod +x "$SERVER_ROOT/scripts/approve.sh"
...
git revert <commit>
EOF
node mission-control/server.mjs --root "$SERVER_ROOT" --port "$PORT" >/tmp/t107-verify-server.log 2>&1 &
SERVER_PID=$!
ready=""
...
TMP_STORE="$(mktemp -d)"
RALPH_STATE_ROOT="$TMP_STORE" ./scripts/pair.sh start >/dev/null 2>&1 && pass "pair.sh start" || fail "pair.sh start"
PT2=$(node --input-type=module -e "import { createPairingToken, exchangePairingToken } from '$ROOT/mission-control/token-store.mjs'; const t=createPairingToken('$TMP_STORE/state/devices'); const d=exchangePairingToken('$TMP_STORE/state/devices', t.token); process.stdout.write(d.device_id);")
RALPH_STATE_ROOT="$TMP_STORE" ./scripts/pair.sh list 2>/dev/null | grep -q "${PT2:0:12}" && pass "pair.sh list" || fail "pair.sh list"
RALPH_STATE_ROOT="$TMP_STORE" ./scripts/pair.sh rename "$PT2" "CLI Test Phone" >/dev/null 2>&1 && RALPH_STATE_ROOT="$TMP_STORE" ./scripts/pair.sh list 2>/dev/null | grep -q "CLI Test Phone" && pass "pair.sh rename" || fail "pair.sh rename"
RALPH_STATE_ROOT="$TMP_STORE" ./scripts/pair.sh revoke "$PT2" >/dev/null 2>&1 && pass "pair.sh revoke" || fail "pair.sh revoke"
