...
def main():
    ...
    ap.add_argument(
        "--segments",
        action="store_true",
        help="세그먼트별로도 합성(presynth 캐시 테스트)",
    )
    ap.add_argument(
        "--segments-only",
        action="store_true",
        help="기존 full mp3는 재사용하고 세그먼트 mp3만 합성",
    )
    args = ap.parse_args()
    ...
    for item in data["items"]:
        ...
        for ver, key in (("A", "A_llm"), ("B", "B_assembled")):
            ...
            if args.segments_only:
                print(
                    f"full 재사용: {seed} {ver} ({len(full)}자) -> {fpath.name}",
                    flush=True,
                )
            ...
            print(
                f"  예측 {pred}s / 실측 {sec:.1f}s / 요금 ${sec / 60 * TTS_PER_MIN:.5f}",
                flush=True,
            )
    ...
    for ver in ("A", "B"):
        ...
        if rs:
            print(
                f"[{ver}] 실측 평균 {statistics.mean(r['actual_sec'] for r in rs):.1f}s, "
                f"평균 요금 ${statistics.mean(r['actual_cost_usd'] for r in rs):.5f}"
            )
    print(
        f"\n결과: {OUT / out_name} / 청취용 mp3는 {OUT}/ 에 저장됨. docs/reports/tts/listening-qa-rubric.md로 평가."
    )
