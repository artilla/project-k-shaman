#!/usr/bin/env bats
# ralph/tests/failures-report.bats — report_failures.sh acceptance criteria tests
#
...
@test "script exists and is executable" {
  [ -f "$REPO_ROOT/ralph/scripts/report_failures.sh" ]
  [ -x "$REPO_ROOT/ralph/scripts/report_failures.sh" ]
}

@test "missing failures.log exits 0 with empty message" {
  run bash "$REPO_ROOT/ralph/scripts/report_failures.sh" --log "$TMP/state/failures.log"
  [ "$status" -eq 0 ]
...
@test "--tail N shows only the last N failures" {
  ...
  run bash "$REPO_ROOT/ralph/scripts/report_failures.sh" --log "$TMP/state/failures.log" --tail 2
  [ "$status" -eq 0 ]
  [[ "$output" == *"T004"* ]]
  [[ "$output" == *"T005"* ]]
  [[ "$output" != *"T001"* ]]
