#!/usr/bin/env bats
# ralph/tests/lint-external-docs.bats — lint_external_docs.sh 회귀 테스트 (T049)
#
# 격리: 각 테스트는 mktemp -d 임시 디렉터리에서 독립 실행.

SCRIPT_PATH="$(cd "$BATS_TEST_DIRNAME/../.." && pwd)/ralph/scripts/lint_external_docs.sh"
...
BASE_BRANCH="${BASE_BRANCH:-$(./ralph/scripts/lib/base_branch.sh)}"
