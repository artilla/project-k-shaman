#!/usr/bin/env bash
# scripts/report_failures.sh — state/failures.log 파서 및 집계 리포트
#
# TSV 포맷 (T009에서 확정): <ISO8601>\t<TICKET_ID>\t<STAGE>\t<RETRY>\t<MESSAGE>
#
# 사용법:
#   ./scripts/report_failures.sh              # state/failures.log 요약
#   ./scripts/report_failures.sh --tail N     # 최근 N개 실패 출력
#   ./scripts/report_failures.sh --log <path> # 로그 파일 경로 지정
