#!/usr/bin/env bats
# tests/orchestrator-failures-log.bats — orchestrator worker failures.log collect regression tests.
#
...
EOF
  ...
  chmod +x "$TEST_HOME/scripts/orchestrator.sh"
...
_run_collect() {
  run env RALPH_TEST_COLLECT_ONLY="$1" \
    bash "$TEST_HOME/scripts/orchestrator.sh"
}
...
@test "after collect, report_failures.sh can parse the appended failure" {
  ...
  run bash "$REPO_ROOT/scripts/report_failures.sh" --log "$TEST_HOME/state/failures.log"
  [ "$status" -eq 0 ]
