#!/usr/bin/env bats
# tests/base-branch.bats — detect_base_branch() 우선순위 6단계 회귀 테스트.
#
...
_run_detect() {
  run bash -c 'cd "$1" && source "$2/scripts/lib/base_branch.sh" && detect_base_branch' \
    -- "$TEST_HOME" "$REPO_ROOT"
}

_run_detect_isolated() {
  run env HOME="$FAKE_HOME" GIT_CONFIG_NOSYSTEM=1 \
    bash -c 'cd "$1" && source "$2/scripts/lib/base_branch.sh" && detect_base_branch' \
    -- "$TEST_HOME" "$REPO_ROOT"
}
