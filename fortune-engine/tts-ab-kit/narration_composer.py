...
CLI:
  python narration_composer.py < fortune.json
"""
import json
import re
import sys
...
FIELD_KO = {"love": "연애운", "money": "금전운", "work": "일과 학업운",
            "relationship": "인간관계운", "condition": "컨디션"}
...
def scores_line(scores: dict) -> str:
    ...
    if scores[low] >= 60:
        ...
        return (f"{josa(top_ko, '이', '가')} {high_phrase[:-2]}고, "
...
def compose_narration(f: dict) -> list:
    ...
    return [
        ...
        {"segment": "lucky", "type": "semi", "text": lucky_line(f["lucky"]["color"], f["lucky"]["item"])},
