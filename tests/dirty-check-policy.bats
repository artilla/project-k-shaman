#!/usr/bin/env bats
# tests/dirty-check-policy.bats — run_loop.sh operational-path dirty policy regression tests.
#
# Verifies:
...
EOF
  ...
  # skills/implementer.md must exist for persona routing
...
EOF
  ...
  # commit so scripts/run_headless.sh is tracked (scripts/ is operational path)
...
# ── Test 2a: operational dirty file (scripts/) -> blocked rc=4 ───────────────

@test "preflight: operational untracked file in scripts/ triggers rc=4" {
  _make_ticket "T092"

  # Operational untracked file in scripts/
  touch "$TEST_HOME/scripts/dirty-helper.sh"
...
@test "preflight: operational modified tracked file in scripts/ triggers rc=4" {
  _make_ticket "T095"

  # Operational tracked file modified after commit
  echo "# dirty" >> "$TEST_HOME/scripts/run_checks.sh"
