#!/usr/bin/env bats
# ralph/tests/safe-false-gate.bats — safe:false approval gate regression tests.
...
setup() {
  ...
  mkdir -p "$TEST_HOME/ralph/scripts" \
           ...
           "$TEST_HOME/ralph/docs" \
           "$TEST_HOME/ralph/mission-control" \
           "$TEST_HOME/state"

  cp "$REPO_ROOT/ralph/scripts/pick_next_ticket.sh" "$TEST_HOME/ralph/scripts/pick_next_ticket.sh"
  ...
  cp "$REPO_ROOT/ralph/scripts/approve.sh" "$TEST_HOME/ralph/scripts/approve.sh"
  cp "$REPO_ROOT/ralph/scripts/ticket_edit.sh" "$TEST_HOME/ralph/scripts/ticket_edit.sh"
  cp "$REPO_ROOT/ralph/scripts/ticket_body.sh" "$TEST_HOME/ralph/scripts/ticket_body.sh"
  # 6라운드 P1(#2): writer lock은 공용 lib — writer들이 source하므로 함께 복사
  mkdir -p "$TEST_HOME/ralph/scripts/lib"
  cp "$REPO_ROOT/ralph/scripts/lib/write_lock.sh" "$TEST_HOME/ralph/scripts/lib/write_lock.sh"
  cp "$REPO_ROOT/ralph/scripts/rollback.sh" "$TEST_HOME/ralph/scripts/rollback.sh"
  # 리뷰 2차 P1-7: run_loop의 safe:false 승인 판정은 ralph/mission-control/approval.mjs 단일 소스.
  cp "$REPO_ROOT/ralph/mission-control/approval.mjs" "$TEST_HOME/ralph/mission-control/approval.mjs"
  cp "$REPO_ROOT/.gitignore" "$TEST_HOME/.gitignore"
  chmod +x "$TEST_HOME/ralph/scripts/"*.sh
...
EOF
  ...
  cat > "$TEST_HOME/ralph/skills/implementer.md" <<'EOF'
# Implementer
EOF
...
EOF
  ...
  cat > "$TEST_HOME/ralph/docs/runbook.md" <<'EOF'
# Runbook
EOF
...
@test "T1: safe:false is skipped in safe-only mode and marked awaiting-approval once" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/pick_next_ticket.sh --safe-only' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
  [[ "$output" == *"[SKIP] T001"* ]]
...
@test "T2: explicit safe:false run without approval marker exits rc=14" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T002' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
@test "T3: explicit safe:false run with valid approval marker completes" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T003' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T4: malformed approval marker exits rc=14 and names invalid fields" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T004' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"approved_by"* ]]
...
@test "T5: safe:true ticket does not require approval marker" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T005' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T6: orchestrator summary marks safe:false branch as human approval required" {
  ...
  cat > "$TEST_HOME/ralph/scripts/run_loop.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
id="$1"
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/orchestrator.sh --max 1' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T7: malformed safe field (safe: yes) -> run_loop rejects fail-closed rc=14" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T007' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"safe 필드가 비정상"* ]]
...
@test "T8: missing/malformed safe field -> picker excludes fail-closed (no awaiting mark)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/pick_next_ticket.sh' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
  [[ "$output" == *"[SKIP] T008"* ]]
  [[ "$output" == *"비정상"* ]]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T009' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"stale"* ]]
...
@test "T10: validator unavailable -> safe:false refused fail-closed rc=14" {
  ...
  rm -f "$TEST_HOME/ralph/mission-control/approval.mjs"

  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T010' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"fail-closed"* ]]
...
EOF
  ...
  # run_loop: 실행 거부
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T011' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T012' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"2"*"1"* ]]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T013' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"unverifiable"* ]]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T014' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"unverifiable"* ]]
}
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T015' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T016' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
EOF
  ...
  run bash -c 'cd "$1" && EDITOR= RALPH_APPROVED_BY=Tester ./ralph/scripts/approve.sh T017' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T018' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"status"* ]]
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T019' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  # picker도 양쪽 모두 후보 제외
  run bash -c 'cd "$1" && ./ralph/scripts/pick_next_ticket.sh' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T20: CRLF ticket -> run_loop rejects and approve --reject fails without touching file" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T020' _ "$TEST_HOME"
  [ "$status" -eq 14 ]

  local before_hash
  before_hash=$(git -C "$TEST_HOME" hash-object docs/tickets/T020-test.md)
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "bad ticket" T020' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "bad closer" T021' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
...
@test "T22: valid ticket --reject succeeds (status skipped, rejection note appended)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "not needed" T022' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T023' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
  [[ "$output" == *"T999"* ]]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T024' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T025' _ "$TEST_HOME"
  [ "$status" -eq 12 ]
  [[ "$output" == *"persona"* ]]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T026' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T27: specific ticket lookup is exact (prefix T999 must not select T9990)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T999 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
...
@test "T28: persona resolved through symlinked skill file -> refused rc=12" {
  ln -s ../docs/master-spec.md "$TEST_HOME/ralph/skills/linked.md"
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T028 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 12 ]
  [[ "$output" == *"symlink"* ]]
...
" "$REPO_ROOT/ralph/mission-control/server.mjs"
  [ "$status" -eq 0 ]
...
@test "T30: symlinked ticket file -> run_loop rc=11 and picker excludes" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T030 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  [[ "$output" == *"symlink"* ]]
...
@test "T31: docs/tickets directory replaced by symlink -> fail-closed everywhere" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T031 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 4 ]
  [[ "$output" == *"symlink"* ]]
...
@test "T32: skills directory replaced by symlink -> persona routing refused rc=12" {
  ...
  mv "$TEST_HOME/ralph/skills" "$TEST_HOME/real-skills"
  ln -s real-skills "$TEST_HOME/ralph/skills"

  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T032 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 12 ]
  [[ "$output" == *"skills"* ]]
}
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "no" T033' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [[ "$output" == *"symlink"* ]]
  # 외부 대상 파일은 무변조
...
@test "T34: dry-run with foreign state/lock preserves it (ownership-scoped cleanup)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T034 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
" "$REPO_ROOT/ralph/mission-control/server.mjs" "$TEST_HOME/nested.md"
  [ "$status" -eq 0 ]
...
EOF
  ...
  ( cd "$TEST_HOME" && ./ralph/scripts/run_loop.sh T036 >/dev/null 2>&1 ) &
  local p1=$!
  ( cd "$TEST_HOME" && ./ralph/scripts/run_loop.sh T036 >/dev/null 2>&1 ) &
  local p2=$!
  local r1=0 r2=0
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T038 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 4 ]
}
...
@test "T39: docs/approvals symlink -> approve refused and validator returns unverifiable" {
  ...
  run bash -c 'cd "$1" && EDITOR= ./ralph/scripts/approve.sh T039' _ "$TEST_HOME"
  [ "$status" -eq 2 ]
  ...
  run node "$TEST_HOME/ralph/mission-control/approval.mjs" "$TEST_HOME" T039
  [ "$status" -eq 6 ]

  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T039' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
}
...
@test "T40: hardlinked ticket -> writer refuses (links>1 guard)" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/ticket_edit.sh set-priority T040 P1' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [[ "$output" == *"hardlink"* ]]
...
@test "T41: CR-contaminated safe line -> shell rejects and server invalidates the field alike" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T041' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
" "$REPO_ROOT/ralph/mission-control/server.mjs" "$TEST_HOME/docs/tickets/T041-test.md"
  [ "$status" -eq 0 ]
...
@test "T42: CR safe line plus normal safe line -> both shell and server treat as duplicate" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T042' _ "$TEST_HOME"
  [ "$status" -eq 14 ]
...
" "$REPO_ROOT/ralph/mission-control/server.mjs" "$TEST_HOME/docs/tickets/T042-test.md"
  [ "$status" -eq 0 ]
...
@test "T44: dry-run in non-git dir cannot be switched to co-pilot via loop_mode" {
  ...
  local ng
  ng="$(mktemp -d)"
  mkdir -p "$ng/ralph/scripts" "$ng/docs/tickets/DONE" "$ng/ralph/skills" "$ng/ralph/docs" "$ng/state" "$ng/ralph/mission-control"
  cp "$TEST_HOME/ralph/scripts/run_loop.sh" "$ng/ralph/scripts/"
  cp "$TEST_HOME/ralph/scripts/run_checks.sh" "$ng/ralph/scripts/"
  cp "$TEST_HOME/ralph/mission-control/approval.mjs" "$ng/ralph/mission-control/"
  cat > "$ng/ralph/scripts/run_headless.sh" <<'EOF'
#!/usr/bin/env bash
echo "MUST-NOT-DISPATCH" >&2
exit 99
EOF
  cat > "$ng/ralph/scripts/pick_next_ticket.sh" <<'EOF'
#!/usr/bin/env bash
ls docs/tickets/T*.md 2>/dev/null | head -1
EOF
  chmod +x "$ng/ralph/scripts/"*.sh
  echo stub > "$ng/ralph/skills/implementer.md"
  echo x > "$ng/docs/master-spec.md"; echo x > "$ng/ralph/docs/runbook.md"
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T044 --dry-run' _ "$ng"
  rm -rf "$ng"
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T450' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T460' _ "$TEST_HOME"
  [ "$status" -eq 11 ]

  # 진짜 증거로 교체하면 충족된다
  printf -- '---\nid: T001\nstatus: done\nsafe: true\n---\n' > "$TEST_HOME/docs/tickets/DONE/T001-fake.md"
  _commit_all "fix done evidence"
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T460 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
}
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T480' _ "$TEST_HOME"
  [ "$status" -eq 16 ]
  [[ "$output" == *"토큰이 교체"* ]]
  # 남의 lock을 지우지 않는다
...
@test "T49: approve --reject publishes status+Rejection atomically; malformed frontmatter leaves file untouched" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "nope" T490' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "nope" T491' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [ "$before" = "$(cksum "$TEST_HOME/docs/tickets/T491-test.md")" ]
}
...
@test "T50: concurrent approval race -> existing marker is preserved, not overwritten" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh T500' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T51: CR-contaminated persona line -> shell rc=12 and server flags persona_malformed" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T510' _ "$TEST_HOME"
  [ "$status" -eq 12 ]
...
" "$REPO_ROOT/ralph/mission-control/server.mjs" "$TEST_HOME/docs/tickets/T510-test.md"
  [ "$status" -eq 0 ]
...
@test "T52: malformed dep values are not promoted; inline comment is tolerated" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T520 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T521 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T522 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && PATH="$1/bin:$PATH" ./ralph/scripts/run_loop.sh T530' _ "$TEST_HOME"
  [ "$status" -eq 16 ]
  [[ "$output" != *"MUST-NOT-DISPATCH"* ]]
  [[ "$output" == *"예약 직전"* ]]
  # 남의 lock 보존
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T540' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [[ "$output" == *"잔존 자손"* ]]
  sleep 7
...
@test "T55: decisions serialize with ticket status - skipped ticket cannot be approved or re-rejected" {
  ...
  # 이미 reject된 상태를 흉내
  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "first" T550' _ "$TEST_HOME"
  [ "$status" -eq 0 ]

  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh T550' _ "$TEST_HOME"
  [ "$status" -eq 3 ]
  [ ! -f "$TEST_HOME/docs/approvals/T550.md" ]

  run bash -c 'cd "$1" && ./ralph/scripts/approve.sh --reject "second" T550' _ "$TEST_HOME"
  [ "$status" -eq 3 ]
...
EOF
  ...
  run bash -c 'cd "$1" && PATH="$1/fakebin:$PATH" ./ralph/scripts/approve.sh --reject "no" T560' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [ "$before" = "$(cksum "$TEST_HOME/docs/tickets/T560-test.md")" ]
  [ -z "$(ls "$TEST_HOME/docs/tickets/".stage.* 2>/dev/null)" ]
}
...
EOF
  ...
  run bash -c 'cd "$1" && PATH="$1/racebin:$PATH" EDITOR=true ./ralph/scripts/approve.sh T580' _ "$TEST_HOME"
  [ "$status" -eq 3 ]
  [[ "$output" == *"동시 승인 경합"* ]]
...
@test "T57: concurrent writers are serialized - no silent lost update" {
  ...
  # rc 기록 전에 중단되지 않는다.
  ( cd "$TEST_HOME" && rc=0 && ./ralph/scripts/ticket_edit.sh set-priority T570 P1 > "$TEST_HOME/w_a.log" 2>&1 || rc=$?; echo "$rc" > "$TEST_HOME/w_a.rc" ) &
  ( cd "$TEST_HOME" && rc=0 && ./ralph/scripts/ticket_edit.sh set-labels T570 "alpha,beta" > "$TEST_HOME/w_b.log" 2>&1 || rc=$?; echo "$rc" > "$TEST_HOME/w_b.rc" ) &
  wait
...
@test "T59: empty-element dependency lists ([,] / [T001,]) are malformed, not dependency-free" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T595 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T596 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
...
@test "T60: DONE evidence with duplicate id/status declarations is not accepted" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T605 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T606 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
...
TKT
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T610 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
if [ ! -f "$TEST_HOME/rej.launched" ]; then
  ...
  ( cd "$TEST_HOME" && ./ralph/scripts/approve.sh --reject "concurrent" T620 > "$TEST_HOME/rej.log" 2>&1; echo \$? > "$TEST_HOME/rej.rc" ) > /dev/null 2>&1 < /dev/null &
  sleep 1
fi
exec "$realdate" "\$@"
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/racebin2:$PATH" EDITOR=true ./ralph/scripts/approve.sh T620' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
WRAP
  ...
  run bash -c 'cd "$1" && PATH="$1/racebin3:$PATH" ./ralph/scripts/ticket_edit.sh set-priority T630 P1' _ "$TEST_HOME"
...
EOF
  ...
  ( cd "$TEST_HOME" && rc=0 && PATH="$TEST_HOME/slowgit:$PATH" ./ralph/scripts/ticket_edit.sh set-priority T640 P1 > "$TEST_HOME/wa.log" 2>&1 || rc=$?; echo "$rc" > "$TEST_HOME/wa.rc" ) &
  local j=0
  ...
  ( cd "$TEST_HOME" && rc=0 && ./ralph/scripts/ticket_edit.sh set-labels T640 "alpha" > "$TEST_HOME/wb.log" 2>&1 || rc=$?; echo "$rc" > "$TEST_HOME/wb.rc" ) &
  wait
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T650' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T670' _ "$TEST_HOME"
  [ "$status" -ne 0 ]
  [[ "$output" == *"id-mismatch"* ]]
  # 완료로 집계되지 않았으므로 telemetry 커밋도 없다
  run git -C "$TEST_HOME" log --format=%s -5
...
@test "T68: writer audit commit does not absorb unrelated pre-staged index entries" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/ticket_edit.sh set-priority T680 P1' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
...
@test "T66: duplicate depends_on declarations are malformed - no bypass via empty first declaration" {
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T661 --dry-run' _ "$TEST_HOME"
  [ "$status" -eq 11 ]
  run bash -c 'cd "$1" && ./ralph/scripts/pick_next_ticket.sh' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
  [[ "$output" != *"T661-test.md"* ]]
...
@test "T69: body edit blocked on the write lock re-checks status after acquiring - skipped ticket is not modified" {
  ...
  ( cd "$TEST_HOME" && rc=0 && printf 'new body\n' | ./ralph/scripts/ticket_body.sh set T690 > "$TEST_HOME/tb.log" 2>&1 || rc=$?; echo "$rc" > "$TEST_HOME/tb.rc" ) &
  local editor=$!
  sleep 0.5
...
EOF
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/run_loop.sh T700' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
  ...
  run bash -c 'cd "$1" && ./ralph/scripts/rollback.sh T700 --yes < /dev/null' _ "$TEST_HOME"
  [ "$status" -eq 0 ]
